...
While inconstancy in spread across the full range of dependent variables can be sometimes be observed in a residual-dependence plot, certain patterns in the data can make such an assessment more challenging in such a plot.

A **spread-location plot** (SL plot) is designed to explore patterns in changes in spreads as a function of increasing $x$ values. The plot pits an expression of spread as a function the fitted values. 
