...
The slope can be used to come up with the best power transformation to minimize the systematic increase in spread. The power to use is 1 - $slope$. The slope can be extracted from the model as using the `coef` function:

```{r}
coef(M)
```

The second value in the output is the slope. So the power to use is 1 - 2.65 or -1.65. We will use the custom Box-Cox power transformation function `bc` (also available in the ES218.R script) to re-express the `mean.length` values. We'll add a new column to `df`:

```{r}
df$bc1.mean.length <- bc(df$mean.length, -1.65)
```

Let's first generate the boxplot of the original values.

```{r fig.height=3, fig.width=4}
boxplot(mean.length ~ dimension, df)
```

Now let's generate the boxplot of the re-expressed values.

```{r fig.height=3, fig.width=4}
boxplot(bc1.mean.length ~ dimension, df)
```

Recall that our goal here was to minimize any relationship between spread ane median. The re-expression seems to have done the trick. 

In his book, Cleveland proposes a power transformation of -1; a power less aggressive than ours. Let's see how well it performs with our data.

```{r fig.height=3, fig.width=6, fig.keep='all'}
...
Neither plot seems to do a bad job in stabilizing spread. To help decide which power to choose, let's see how well they help balance the spread about the median.
