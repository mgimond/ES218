...
```{r fig.height=2, fig.width=2.2, echo=2:10}
...
```{r fig.height=2, fig.width=2.2, echo=2:5}
OP <- par(mar=c(2.4,2.4,0,1), cex.axis=0.8, mgp=c(1.3,0.5,0))
plot(sprd ~ med, df.sl, pch=16)
...
The second value in the output is the slope. So the power to use is `1 - 2.65` or `-1.65`. We will use the  power transformation function `BC`  to re-express the `mean.length` values. We'll add the re-expressed values as a new  column to `df`:

```{r}
df$re.mean.length <- BC(df$mean.length, -1.65)
```
...
We'll check for a homogeneous spread across fitted medians using the original spread-level plot covered in last week's [lecture](file:///C:/Users/mgimond/Documents/GitHub/ES218/Week07b.html#constructing-the-s-l-plot).
