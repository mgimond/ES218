...
The s-l plot suggests that the residuals increase monotonically with an increase in the fitted median value. Note that the x-axis is not categorical, it shows the median values for each species group. 
...
Increasing or decreasing spreads as a function of fitted value can, for in some instance, be corrected by re-expressing the continuous variable. For example, applying a log transformation, helps stabilize the residuals as can be seen in the following s-l plot. 
...
As with many `tukeyedar` functions used in the course, the power transformation applied to the data is shown in the upper land hand corner. By default, the power transformation is `1` (i.e. an untransformed data).
...
The following code block demonstrates the steps required to create an s-l plot.
...
For a `ggplot` implementation of the this variant of the s-l plot, the R as follows.
