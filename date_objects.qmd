...
`lubridate` has functions that will create *time* objects that are of `period` class. For example, the following will generate a `period` object from the string `"3:15"`.

```{r}
hm("3:15")
```

Other period parsing functions include:
...
The CO2 dataset has the date split across two columns: `Year` and `Month` (both stored as integers). You can combine the columns into a character string using the `paste` function. For example, if we want to create a "Year-Month" string as in `1959 10`, we can type:

```{r eval=FALSE}
paste(dat1$Year, dat1$Month)
```

The above example uses as arguments the values that are to be pasted together (`Year` and `Month` in this example). Note that the function can paste more than two values.

By default, the `paste` function adds a space between the two values. If a different separator is desired, then the `sep` argument can be added to the function. For example, to separate the values using a dash `-`, type:

```{r eval=FALSE}
paste(dat1$Year, dat1$Month, sep = "-")
```

The `lubridate` parsing functions will accept different non-numeric delimiters, including spaces.

For the CO2 dataset, we will use `lubridate`'s `ym` parsing function.

```{r eval=FALSE}
ym(paste(dat1$Year, dat1$Month))
```

We are now ready to add a new column called `Date` to the `dat` object and fill that column with a formal *Date* object:

```{r}
dat1$Date <- ym(paste(dat1$Year, dat1$Month))
head(dat1)
```

Note that the date object is formatted as *Year-Month-Day* in the output--this is R's default date format.
...
You'll note that since a day was not specified, the function defaulted to the first day of each month. If you prefer to center the date on each month, you can add a constant to the parsing function. For example, to center each month on the 15^th^ day of each month, type:

```{r val = FALSE}
dat1$Date <- ymd( paste(dat1$Year, dat1$Month, "15") )
```

Note that because we did not add a *timezone* or a *time* component to the date object, the `Date` column was assigned a `Date` class as opposed to the `POSIX...` class. 
...
The `month()` and `wday()` have a `label` option that will output the values as text and not number. For example:
...
You can add and subtract day/time to a date object using `days()`, `hours()`, `minutes()` or `seconds()` functions. For example, to add a full day to the `x.date` vector elements, type:
...
--------------|--------------------------|------------------------------------------
`%a`          | Abbreviated weekday name | `r format(x.date, format="%a")`
`%A`          | Full weekday name        | `r format(x.date, format="%A")`
`%m`          | Month as decimal number  | `r format(x.date, format="%m")`
`%b`          | Abbreviated month name   | `r format(x.date, format="%b")`
`%B`          | Full month name          | `r format(x.date, format="%B")`
`%c`          | Date and time, locale-specific |  `r format(x.date, format="%c")`
`%d`          | Day of the month as decimal number | `r format(x.date, format="%d")`
`%H`          | Hours as decimal number (00 to 23) | `r format(x.date, format="%H")`
`%I`          | Hours as decimal number (01 to 12) | `r format(x.date, format="%I")`
`%p`          | AM/PM indicator in the locale | `r format(x.date, format="%p")`
`%j`          | Day of year as decimal number | `r format(x.date, format="%j")`
`%M`          | Minute as decimal number (00 to 59) | `r format(x.date, format="%M")`
`%S`          | Second as decimal number | `r format(x.date, format="%S")`
`%U`          | Week of the year starting on the first Sunday | `r format(x.date, format="%U")`
`%W`          | Week of the year starting on the first Monday | `r format(x.date, format="%W")`
`%w`          | Weekday as decimal number (Sunday = 0) | `r format(x.date, format="%w")`
`%x`          | Date (locale-specific)  | `r format(x.date, format="%x")`
`%X`          | Time (locale-specific)  | `r format(x.date, format="%X")`
`%Y`          | 4-digit year            | `r format(x.date, format="%Y")`
`%y`          | 2-digit year            | `r format(x.date, format="%y")`
`%Z`          | Abbreviated time zone   | `r format(x.date, format="%Z")`
`%z`          | Time zone               | `r format(x.date, format="%z")`
