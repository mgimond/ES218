...
```{r clock, fig.width=9, fig.height=2.2, warning=FALSE, message=FALSE, echo=FALSE, results='hide'}
...
cnames <- c("Effects","ed8","ed9to11","ed12","ed13to15","ed16" )
iter.fac <- c("Raw Table", paste("Iteration #",1:4))
...
df2.l <- df2  %>%
  ...
  mutate( Regions = factor(Regions, levels=rev(rnames)),
          Edu = factor(Edu, levels=cnames),
          Iteration = factor("Raw Table",levels=iter.fac))

# Loop through each median polish iteration
for (i in 1:3)  {
  M1 <- medpolish(as.matrix(df2[-1,-1:-2]), maxiter = i)
  dfa <- rbind( t(as.data.frame(M1[3]))  , as.data.frame(M1$residuals) )
  dfb <- cbind( Regions    = factor(rnames,levels=rev(rnames)), Effects=c(M1$overall, M1$row) , dfa)
  df2.l <- gather(dfb, key=Edu, value = Deaths, -Regions) %>% 
    mutate(  Edu = factor(Edu, levels=cnames),
             Iteration = factor(paste("Iteration #",i),levels=iter.fac)) %>% 
    bind_rows(df2.l)
}

# Plot the tables side-by-side
  ggplot(df2.l) + aes(x=Edu, y=Regions, fill=Deaths) +
    geom_tile(colour="grey", show.legend=FALSE) +
    geom_text(aes(fill = Deaths, label = round(Deaths, 1))) +
