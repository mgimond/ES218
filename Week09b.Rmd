...
Recall that we are seeking to minimize an non-random pattern in the residual. Even though a monotonic increase in residuals is discernible in both plots, the 3-point summary residual has less of a slope and (ignoring the outlier) has a smaller spread.
...
In this example, we fit both a un-modified regression line (dashed grey line) and a bisquare modified regression line (red line) to yearly mean temperature values for the Gulf of Maine.
...
The bisquare estimation method can also be extended to the loess smoothing function. The following chunk of code fits both an uncorrected robust loess (dashed curve) and a bisquare loess (red curve) the the Gulf of Maine temperature data.
...
ggplot(df2) + aes(x=Year, y=AvgTemp) + geom_point() + 
              stat_smooth(family="symmetric", se=FALSE, span=1/3, col="red") +  # Robust loess
