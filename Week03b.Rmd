...
The `pivot_longer` function can also be used in a piping operation as follows:

```{r eval = FALSE}
df %>% 
  pivot_longer(cols=c(Q1,Q2,Q3,Q4), names_to = "Quarter", values_to = "Delay")
```
...
The `pivot_wider` function can also be used in a piping operation as follows:

```{r eval = FALSE}
df.long %>% 
  pivot_wider(names_from = Quarter, values_from = Delay) 
```
<p></p>
