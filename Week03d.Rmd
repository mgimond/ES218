...
We've already seen how the conditional statements can be used to check whether a variable is equal to, less than or greater than a number. We can use conditional statements to evaluate if a variable matches an exact string. For example,
...
returns `TRUE` since the strings match exactly. However, note that **R differentiates cases** so that the following query,

```{r}
a == "abc def"
```

returns `FALSE` since the first character does not match in case (i.e. upper case `A`vs lower case `a`).

### How to ignore case sensitivity?

If you want to R to ignore cases in any string operations, simply force all variables to a lower case and define the pattern being compared against in lower case. For example:
...
To check if object `a` has the pattern `c d` anywhere in its string, we can use the `str_detect` function as follows:
...
To check if object `a` *ends* with the pattern `Abc` add the dollar character `$` at the end of the pattern as in:
...
If you want to find where a particular pattern lies in a string, use the `str_locate` function. For example, to find to find where the pattern `c d` occurs in object `a` type:
...
Note too that the `str_locate` function only returns the position of the first occurrence. For example, 
...
will only return the start/end positions of the first occurrence of `Ab`. To find *all* occurrences, use the `str_locate_all()` function as in:
...
You can remove leading, ending or both white spaces from a string. For example, to remove leading white spaces from object `d` type,
...
Note that if object `g` consists of more than one element, the output will be a list of elements and not a vector of elements.

## Extracting parts of a string that follow a pattern

To extract the three letter month from object `g` (defined in the last example), you can use a combination of `stringr` functions as in:
...
The above chunk of code firsts identifies the position of the `Month:` string and passes its output to the object `loc`. It then uses the `end` position in the call to `str_sub` to extract the three characters making up the month abbreviation. The value `1` is added to the `start` parameter to omit the last character of `Month:` (recall that the `str_locate` positions are inclusive).

This can be extend to multi-element vectors as follows:

```{r}
gs <- c("Year:2000, Month:Jan, Day:23",
       "Year:345, Month:Mar, Day:35",
       "Year:1867 , Month:Nov, Day:35")
