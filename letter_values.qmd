...
# Beyond the Boxplot: Tukey’s Letter-Value Summaries
...
In Chapter 17, we introduced the boxplot as a five-number summary that captures the center, spread, and potential outliers of a distribution. While useful, the boxplot only summarizes a few key quantiles. In this chapter, we extend that idea using Tukey’s letter-value summaries, which provide a more detailed view of a distribution’s symmetry and tail behavior.
...
Letter-value summaries are especially useful when you want to assess distributional symmetry or detect subtle skewness that may not be visible in a boxplot. They are also helpful for comparing batches of data where tail behavior matters.
...
This final chapter introduced Tukey’s letter-value summaries as an extension of the traditional five-number summary used in boxplots. You learned how to compute and interpret deeper quantiles (e.g., eighths, sixteenths) to assess distributional symmetry and tail behavior. Letter-value summaries offer a more detailed view of a batch’s structure and are especially useful when subtle asymmetries or outliers may influence analysis.
