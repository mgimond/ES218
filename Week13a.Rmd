...
Variables can be mapped in R using readily available CRAN packages such as `rgdal` and `maps`. The focus of this tutorial will be on mapping vector (aka areal) data and not on raster (aka field) data.
...
Mapping values requires two datasets: the values to be mapped and the spatial boundaries of the areas being mapped. Packages such as `maps` offer R-ready maps for the US states and counties and administrative boundary maps of other countries such as Canada, France and Italy. 

In the following example we will use `ggplot` to draw the counties map. We will also color the counties based on the state they are located in. The state name column is labeled `region` and the county name column is labeled `subregion`.
...
## Joining income table to the map by county and state

The next step is to *join* a dataset to the counties map. We will use the income-by-gender-education dataset used earlier in the course and limit the data to *median income per person*.
...
You notice that the counties map does not seem complete; most notable is the absence of all Louisiana counties. Let's look at the rows in `cnty.df1` associated with the state of Louisiana.
...
Now that the FIPS codes are part of the county map dataframe, let's join the income data table then map the income values.
...
This is an improvement over the last map. But there are still a few counties missing. After a closer scrutiny of the the data, it seems that the `county.fips` table splits some counties into two or more sub-regions. For example, Accomack county (Virginia) is split into two regions (and thus into two different names):

```{r}
county.fips %>% filter( grepl("^virginia,accomack", polyname) )
```

Fixing these discrepancies would require manual intervention and time. Another work around is to use the Census Bureau's map files instead of `maps`'s built-in map.
...
Next we can append the original census map table to the new `cnty2` object.
...
The `labels=` parameter allows us to define the output interval names. Here we assign the interval's top dollar value to the intervals names. Note that we could have created fancier intervals such as `$0 to $20k`, `$20k to $30k`, etc...

Next we'll map the data, but first we need to change the coloring scheme since we are now using a *discrete* color scheme as opposed to a *continuous* one. Cynthia Brewer of Penn State designed a website called [http://colorbrewer2.org/](http://colorbrewer2.org/) that suggests discrete color schemes to use for different map effects. The choice of colors, especially for a sequential color scheme where the hue is kept constant while its lightness level is modified, is not trivial. Fortunately, her color scheme recommendations are available in `ggplot` via the `scale_fill_brewer()` function. To see a list of Brewer color schemes, simply type `display.brewer.all()` in the console window. Note that we will need to load the `RColorBrewer` package first.
...
We might choose to map the income distribution using a **divergent** color scheme where we compare the counties to some overall value such as the county median. We can use the `quantile()` function to break the income data into its percentiles. This will be helpful when generating symmetrical intervals about the median.
...
A divergent color scheme consists of two different hues whose lightness values converge towards some central value (the median income in our case). We use Brewer's red-to-green color palette `RdYlGn` for this next map.
