...
We learned in the last chapter that one version of the spread-level plot can suggest the power transformation to use. We'll recreate the plots (however this time, we'll make use if the base plotting functions).
...
Recall that our goal here was to minimize any systematic relationship between spread and median. The re-expression seems to have eliminated the monotonic increase in spread, but an s-l plot will be more effective in helping confirm this.
...
OP <- par(mar=c(2,2,0,1), bty='n', cex.axis=0.8, mgp=c(1.5,0.5,0))
library(tukeyedar)
boxplot( eda_re(mtcars$mpg, p=-1), horizontal = TRUE)
par(OP)
```
...
OP <- par(mar=c(2,2,0,1), bty='n', cex.axis=0.8, mgp=c(1.5,0.5,0))
boxplot( eda_re(mtcars$mpg, p=-1, tukey = FALSE), horizontal = TRUE)
par(OP)
```
