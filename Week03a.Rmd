...
Another approach to combining `dplyr` operations is to use the **piping operator** ,`%>%`, which daisy chains `dplyr` functions. So our previous workflow would look like:
...
## Computing the residuals after fitting a linear regression model for each group

(Note that this example covers topics best suited for advanced users)

This is an extension of the last example where, instead of fitting a constant (median) value to the yield values within each crop group, we fit a linear regression line to crop yield vs. year within each crop group, then compute the differences between the line and the values thus giving us the *residuals*.
...
dat2 <- dat %>% 
  ...
  select(Crop, Year, Value)                    %>%
  group_by(Crop)                               %>%
  mutate(Residuals = Value - predict(lm(deparse(substitute(Value ~ Year))),
                                     data.frame(Year)) )   
```

The `deparse(substitute(...))` functions are necessary in the piping operations for `dplyr` to properly interpret the formula. The `predict()` function extracts predicted values from the regression model by passing one or many values as a dataframe (even if we are only predicting the crop yield for a single year by row).  
