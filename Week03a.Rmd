...
```{r error=TRUE}
case_when(z == "deg" ~ NA,  # NA is a logical type, this will return an error
          z == "F" ~ "Fahrenheit",
          z == "C" ~ "Celsius",
          TRUE ~ z)
```
In this example, `case_when` requires a character `NA` type. We can use either `as.character(NA)` or `NA_character_`.
