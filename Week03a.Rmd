...
met2
```
The dot `.` is a placeholder for each column listed in the `list()` function. Also, note the `~` operator in lieu of `. =`.
...
`recode` replaces values but it will not allow for more complex operations. For example, given two vectors, `unit` and `temp`, we would like to convert all `temp` values to Fahrenheit by applying a temperature conversion dependent on the `unit` value.
