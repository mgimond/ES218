---
title: "Week 3: Querying data tables"
output:
  html_document:
    toc: yes
    toc_depth: 3
...
The `deparse(substitute(...))` functions are necessary in the piping operations for `dplyr` to properly interpret the formula. The `predict()` function extracts predicted values from the regression model by passing one or many values as a data frame (even if we are only predicting the crop yield for a single year by row).  
