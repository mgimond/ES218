...
At this point, R treats the vector `x` as characters. To force R to interpret these as dates, use `lubridate`'s `mdy` function.
...
The function `dmy` can read in date formats that use different delimiters so that `mdy("06/23/2013")`, `dmy("06-23-2013")` and `dmy("06.23.2013")` are parsed exactly the same. Different `lubridate` functions are available for different date formats. A few are listed below.
