...
This chapter will make use of a dataset housed on this course's website.

```{r}
df <- read.csv("http://mgimond.github.io/ES218/Data/Sample1.csv")
```

Next, a linear model is fitted to the data.
...
The line seems to do a decent job in depicting the *overall* trend but the relationship does not appear perfectly linear.

Let's check the residuals via a residual-dependence (R-D) plot.
...
The segmented plots seem to confirm our earlier suspicion that the data followed three separate linear processes. We can extract the slope and intercept for each segment using the following code chunk:
...
In the above example, three separate models were needed to capture (what we believed to be) three different underlying processes.  
