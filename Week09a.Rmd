...
Bivariate data are datasets that store two variables measured from a same observation (e.g. wind speed **and** temperature at a single location). This differs from univariate data where only one variable is measured for a single observation (e.g. temperature at a single location). 

# Scatter plot

A scatter plot is a popular visualization tool used to compare values between two variables. Sometimes one variable is deemed *dependent* on another variable, the *independent* variable; Cleveland refers to the former as the *response*  and the latter as the *factor*. The dependent variable is usually plotted on the y-axis and the dependent variable is usually plotted on the x-axis. Other times, one does not seek a dependent-independent relationship between variables but is simply interested in studying the relationship between them.
...
bank <- function(x,y,deg=45) {
  seg.ang <- atan( diff(y) / diff(x) ) * 180/pi
  seg.len <- sqrt( diff(y)^2 + diff(x)^2 )
  WMD <- sum(abs(seg.ang) * seg.len) /  sum(seg.len)  
  aspect <- tan(abs(deg) * pi/180) / tan(WMD * pi/180)
  return(aspect)
}
```
...
```{r fig.width=2.5, fig.height=2.5}
library(ggplot2)
ggplot(df) + aes(x=area, y=cp.ratio) + geom_point() + stat_smooth(method="lm", se=FALSE)
```
...
A second order polynomial is a three parameter function ($a$, $b$ and $c$) whose equation $y = a + bx + cx^2$ defines the curve that best fits the data. We can define the equation formula as `cp.ratio ~ area + I(area^2)`,
...
The LOESS function can be computed using the `loess()` function. It takes as arguments  `span` ($\alpha$), and `degree` ($\lambda$).
...
```{r fig.width=2.5, fig.height=2.5}
ggplot(df) + aes(x=area, y=cp.ratio) + geom_point() + 
             stat_smooth(method="loess", se=FALSE, span=0.5, degree=1 )
```

# Residuals

Fitting the data with a line is just the first step in EDA. Your next step should be to explore the residuals. The residuals are the distances (along the y-axis) between the observed points and the fitted line. The closer the point to the line (i.e. the smaller the residual) the better the fit. 

The residuals can be computed using the `residuals` function. It takes as argument the model. For example, to extract the residuals from the linear model `M` computed earlier type, 
...
We'll create a **residual dependence plot** to plot the residuals as a function of the x-values. We'll do this using `ggplot` so that we can also fit a loess curve to help discern any pattern in the residuals (the `ggplot` function makes adding a loess smooth easy).
...
ggplot(res1) + aes(x=area, y=residuals) + geom_point() +
               geom_abline(c(0,1)) + 
               stat_smooth(method="loess", se=FALSE, span=1, degree=1 )
```

We are interested in identifying any pattern in the residuals. If the model does a good job in fitting the data, the points should be uniformly distributed across the plot. With the linear mode, we observe a convex pattern in the residuals suggesting that the linear model is not a good fit. We say that the residuals show *dependence* on the x values. Let's look at the second order polynomial model `M2`,
...
There is no indication of dependency between the residual and the `area` values. The second order polynomial is an improvement. Let's look at the loess model.
...
Next we will test for homogeneity in the residuals.

## Spread-location plot

The `M2` and `lo` models do a good job in eliminating any dependence between residual and x-value. Next, we will check that the residuals do not show a dependence with *fitted* y-values. This is analogous to univariate analysis where we checked if residuals increased or decreased with increasing medians across factors. Here we will compare residuals to the fitted `CP.ratio` values (think of the fitted line as representing a *level* across different segments along the x-axis). We'll generate a spread-level plot of model `M2`'s residuals (note that in the realm of regression analysis, such plot is often referred to as a **scale-location** plot). We'll also add a loess curve to help identify any patterns (this reproduces fig 3.14 in Cleveland's book).

```{r fig.height=2.5, fig.width=2.5, small.mar=TRUE}
sl2 <- data.frame( std.res = sqrt(abs(residuals(M2))), fit = fitted.values(M2))
ggplot(sl2) + aes(x=fit, y=std.res) + geom_point() +
               geom_abline(c(0,1)) + 
               stat_smooth(method="loess", se=FALSE, span=2, degree=1 )
```

The function `fitted.values()` extracts the y-values from the fitted model `M2`. It's clear from this plot that the residuals are not homogeneous; they increase as a function of increasing *fitted* CP ratio. Note that we would observe this monotone spread with our loess model as well.

# Re-expressing the data

The monotone spread can be problematic if we are to characterize the spread of `cp.ratio` the same across all values of `area`. To remedy this, we can re-express the `cp.ratio` values. Ratios are good candidates for log transformation. We will therefore fit a new linear model to the data after transforming the y-value.
...
```{r fig.height=2.5, fig.width=2.5, small.mar=TRUE}
sl3 <- data.frame( std.res = sqrt(abs(residuals(M3))), fit = fitted.values(M3))
ggplot(sl3) + aes(x=fit, y=std.res) + geom_point() +
               geom_abline(c(0,1)) + 
               stat_smooth(method="loess", se=FALSE, span=2, degree=1 )
```

We do not observe a systematic increase in spread, the log transformation seems to have removed the monotone spread as well. 
