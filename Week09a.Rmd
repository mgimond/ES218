...
```{r fig.width=2.5, fig.height=2.5}
ggplot(df, aes(x=area, y=cp.ratio)) + geom_point() + 
             stat_smooth(method="loess", se=FALSE, span=0.5, method.args = list(degree=1))
```
...
You may ask "if the loess model does such a good job in fitting dat, why bother with polynomial fits?". If you are seeking to generate a predictive model that explains the relationship between the y and x variables, then a mathematcially tractable model (like a polynomila model) should be sought. If the interest is simply in *seeing* a pattern in the data, then a loess fit is a good choice.

You may ask *"if the loess model does such a good job in fitting data, why bother with polynomial fits?"*. If you are seeking to generate a predictive model that explains the relationship between the y and x variables, then a mathematically tractable model (like a polynomial model) should be sought. If the interest is simply in *seeing* a pattern in the data, then a loess fit is a good choice.
