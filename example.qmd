...
---
# A deeper exploration of residuals
...
An convex trend is present in the residual. This suggests that we should augment the model to a 2^nd^ order polynomial of the form:
...
This does not appear to provide an improvement over the 2^nd^ order polynomial fit. At this point, we could experiment with higher-order polynomials to better capture the trend, or we might consider shifting to a non-parametric approach.
...
At first glance, this may not look any different from our 2^nd^ or 3^rd^ order polynomial models. But the resulting residuals suggest that the loess smooth did a better job in removing any decadal patterns in our batch of CO2 values.
