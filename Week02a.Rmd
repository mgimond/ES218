...
Note that removing the fraction part of a number when creating a numeric object does not necessarily create an integer. For example, creating what seems to be an integer object return double when queried by `typeof()`:
...
To force R to recognize a value as and integer add an upper case `L` to the number.
...
Characters can also consist of numbers represented as characters. The distinction between a numeric representation of a number and a numeric one is important. For example, if we have two numeric vectors `x` and `y` such as
...
**Factors** are usually used to group variables into fixed number of **levels**. For example, a dataset may be grouped by gender or month of the year. Such data are usually loaded into R as a numeric or character data type requiring that they be converted to a factor using the `as.factor()` function.

In the following example, a dataframe is created with a column storing one of two values: `M` and `F`. These values are first encoded as character objects, but are then converted to factors using the `as.factor` function.
...
The `gender` column is no a factor with two levels: `F` and `M`. 
...
Many functions recognize factor data types and will allow you to split the output into groups defined by the factor's unique levels. For example, to create two boxplots of the value `x`, one for each gender type `F` and `M`, type the following:
...
A factor will define a hierarchy for its levels.  When we invoked the `levels` function in the last example, you may have noted that the level `F` appeared before `M`or  --this is the level hierarchy defined by R (i.e. `F`>`M` ). This means that regardless of the order in which the factors appear in a table, anytime a plot or operation is conditioned by the factor, the grouped elements will appear in the order defined by the levels. When we created the boxplot from our `dat` object, the plotting function ordered the boxplot (bottom to top) following `gender`'s level hierarchy (i.e. `F` first, then `M`). 

If we wanted the boxplots to be plotted in a different order (i.e. `M` first then `F`) we would need to modify the `gender` column by recreating the factor object as follows:
...
The `factor` function is given the original factor values but is also given the levels in the order in which they are to appear. Now, if we recreate the boxplot, the `M` plot will appear first (closest to the y=0 origin).
...
We can subset the table by level using the subset function. For example, to subset the values associated with `F`, type:

```{r}
dat.f <- subset(dat, gender == "F")
dat.f
```

However, if you display the levels associated with this new dataframe, you'll see that level `M` is still listed even it no longer appears in the `gender` column.

```{r}
str(dat.f)
```

This can be nuissance when plot the data subset.
...
Even though no records are available for `M`, the plot function allocates a slot for that level. To resolve this, we can use the `droplevels` function to remove all unused levels.
...
You will find that many data files contain missing or unknown  values. It may be tempting to assign these missing or unknown values with `0` but doing so can lead to many undesirable results when analyzing the data. R has two placeholders for such elements: `NA` and `NULL`.
...
You can view each element's structure using the `str()` function.

```{r}
str(lst)
```

Each element of a list can be extracted using the `$` symbol followed by that element's **tag** (the name that was assigned to each element when we constructed the list object). For example, to access element `A` from list `lst`, type:

```{r}
lst$A
```

You can also access that same element using its *numerical index*. Since `A` is the first element in `lst`, its numerical index is `1`.
...
The tag names for each element in a list can be extracted using the `names()` function:

```{r}
names(lst)
```

Note that lists do not require tag names for its elements. For example, we could have created a list as follows:

```{r}
lst.notags <- list( A, B, D)
```

When we list its contents, we will see bracketed indices instead of tags.
...
It's usually good practice to assign tags to list elements as these can provide meaningful descriptions of each element.
...
The contents of the regression model object `M` consists of `r length(M)` elements which include various diagnostic statistics, regression coefficients, and residuals. Let's extract each element's tag (i.e. element name):

```{r}
names(M)
```

Fortunately, the regression function assigns tags to each of its output's elements making it easier to figure out what most of these elements represent. For example, it's clear that the `residuals` tag stores the residual values from the regression model.
...
So if we want to access the element `rank` in the element `qr` of list `M`, we can type:

```{r}
M$qr$rank
```

If we want to access `rank` using indices instead, and noting that `qr` is  the 7th element in list `M` and that `rank` is the 5th element in list `qr` we type:
