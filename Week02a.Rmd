...
Date values can be represented in tables as numbers or characters. But to work properly with date representation, it must be converted to an R **date** object. R provides many facilities to convert and manipulate dates and times, but a package called `lubridate` makes working with dates/times much easier. A separate [section](Week02c.html) is dedicated to the manipulation of date objects.

# Data structures

A data type defines a single element (e.g. a single number or a single date), but most datasets we work with consist of many elements such as a table of temperature values or a list of survey results. These datasets are stored in R in one of many *data structures*: **vectors**, **matrices**, **data frames** and **lists**.
...
Note that a vector can be one data type only. For example, you cannot mix numeric and character types as in:
...
Matrices and arrays can store numeric or character data types, but they cannot store both in a same object as with vector objects. This is not to say that you can't have a matrix of the kind
...
A **list** is an ordered set of elements stored in a 1D structure.  In fact, it's a another kind of vector called a **recursive vector**  where each element (referred to as a **slot**) can be of different data type and *structure*.  This implies that each element of a list can hold complex objects such as matrices, data frames and other list objects too! Think of a list as a single column spreadsheet where each cell (or slot) can store anything from a number to a three paragraph sentence to a five column table. The latter may be difficult to implement in a spreadsheet be can be readily implemented in a list.
