...
### Using `class` to identify a factor

The `mode` or `typeof` functions will not tell you if an object is a factor.

```{r}
typeof(a.fact)
```

Instead, it's best to use the `class` function as in,

```{r}
class(a.fact)
```

This is because a factor stores its values as integers that point to the levels stored in the vector's attribute header. More details are provided later in this chapter.
