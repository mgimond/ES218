...
The output does not look like a numeric representation of the original elements in `numfac`. Instead it lists the order in which the unique values (or **levels** in R lingo) are stored in the vector's attributes. To see the *unique* levels in our factor `numfac`, type:

```{r}
levels(numfac)
```

There are three unique values in our vector. The order in which the levels appear in the above output represents the ordered *tag* values. So tag number `1` repesents level `100.01`, tag number `2` represents level `23.4`, and tag number `3` represents level `6`. So our earlier attempt at converting the factor to a number ended up outputing the tag values instead.

![](img/levels.png)

So, to extract the values (and not the tags), you must first convert the factor to character before converting to a numeric vector.
...
There are many coercion functions in R, a summary of some the most common ones we'll be using in this course folows:
