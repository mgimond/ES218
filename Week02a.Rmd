...
**Factors** are usually used to group variables a fixed number of levels. For example, a dataset may be grouped by gender or month of the year. Such data are usually loaded into R as a numeric or character data type requiring that they be converted to a factor using the `as.factor()` function.
...
Many functions recognize factor data types and will allow you to split the output into groups defined by the factor's levels. For example, to create two boxplots of the value `x`, one for each gender type `F` and `M`, type the following:
