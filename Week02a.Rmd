...
The output does not look like a numeric representation of the original elements in `numfac`. This is because a factor *tags* the unique values (or **levels** in R lingo). To see the *unique* levels in our factor `numfac`, type:
...
So, to extract the levels (and not the tags), you must first convert the factor to character before converting to a numeric vector.
