...
### `slice`: Extract rows by index

You can extract specific rows by index using the `slice()` function. This function takes a vector of row indices as an argument. For example, to extract rows 2, 3, 4, and 10 from `mtcars`, use:
...
To enumerate rows in a dataframe, use `row_number`. For example, to enumerate `met` data by their wind value from lowest to highest, first sort the data by `Wind`, then create a new column to store the assigned row numbers in a mutate function.

```{r}
wind.sort <- arrange(met, Wind)
mutate(wind.sort, row.num = row_number())
```
...
## A note about vectorized functions

Vectorized functions in R operate on entire vectors, but **not all vectorized functions behave the same way in `mutate()`**. Some, like `+`, work **element-wise**, while others, like `sum()`, return a **single aggregated value**.

For example, the following does **not** sum `x` and `y` row by row:
...
The function `sum(x, y)` sums *all* values in x and y together returning a single number. Since `mutate()` expects a vector of the same length as the number of rows (five in this working example), it recycles that single number across all rows.

Common aggregating vectorized functions include `sum()`, `mean()`, `min()`, `max()`, `median()`, `sd()`, `var()` and `IQR()`.

If you want to apply an aggregating function row by row, use `rowwise()`:

```{r}
df %>% rowwise() %>% mutate(z = sum(x,y))
```