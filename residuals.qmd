...
The means clearly differ across the groups, but you’ll also notice that the range of singer heights tends to cluster around their respective group means. In other words, the range in height values is not consistent across all groups. For instance, `Alto 2` singers have heights ranging from `63` to `70` inches, while the `Bass 1` group spans from `66` to `75` inches--a range of around `9` inches. 
