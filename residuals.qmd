...
eda_jitter(df, height,  show.par = FALSE, xlab = "All singers")
```
...
## Comparing the residuals with QQ plots
...
$$
Here, the subscript $_{voice\ part}$ indicates that the residuals are calculated separately for each group.
...
## What do we gain with homogeneous residuals?
...
Since the residuals appear homogeneous across groups, we can pool them to calculate a single IQR for all singers, thereby reducing variability:
...
## Comparing residuals to a unit Normal distribution
