...
If the file resides on a website, you can load the CSV directly from that site as follows:
...
R has its own file  format; it's usually saved using the *.rds* extension. To read a R data file, invoke the `readRDS()` function.
...
Likewise, to import an SPSS file, use the `spss.get()` function; to import a STATA file, use the `stata.get()` function.
