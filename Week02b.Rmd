...
A package that does a good job in importing Excel files is `readxl`. It usually recognizes the data format defined by Excel (this includes dates). However, only one sheet can be loaded at a time. So if multiple Excel sheets are to be worked on, each sheet will need to be loaded into separate dataframes.

If you don't have the `readxl`library installed, install the package as you would any other package:
```{r eval=FALSE}
install.packages("readxl")
```

In this example, we will load an Excel sheet called `Discharge` which tabulates daily river water discharge. The sample file can be downloaded [here](http://mgimond.github.io/ES218/Data/Discharge_2004_2014.xlsx). The following chunk of code assumes that the Excel file is saved in a folder called `Data`. 
...
Note that the date field was saved in a date format in the original Excel file.
