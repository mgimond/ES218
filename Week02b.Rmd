...
The .rds file format is usually smaller than its text file counterpart and will therefore take up less storage space. The .rds file will also preserve data types such as factors and dates eliminating the need to redefine data types after loading the file.
...
Instead of downloading the file into virtual memory, R needs to download the file into a temporary folder before it can open it. However, that temporary file my not be available in a later R sessions.
...
To save only two R session objects--`dat` and `dat.sub`--to a file, pass the list of object to the `list=` parameter:
