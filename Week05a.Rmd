...
A histogram **bins** the values (usually in equal sized bins) and plots the frequency in a which each *bin* is filled. For example, to create a boxplot of batch `a` where each bin size covers one unit, we type:

```{r fig.height=2, fig.width=4.5}
ggplot(df) + aes(x=a) +geom_histogram(breaks = seq(6.5,16.5,by=1), colour="white")
```

We have two values that fall in the first bin (bin ranging from 6.5 to 7.5), two more value that fall in the second bin (bin value ranging from 7.5 to 8.5) and so on until the second to last bin where we have 3 values that fall in it (bin covering the range 14.5 to 15.5). Note in the parameter `seq(6.5, 16.5, by=1)` we are defining the last bin as covering a range from 15.5 to 16.5 which ends up not receiving a value, hence to grey bar on the plot at that location.
...
The boxplot provides us with many meaningful pieces of information. For example, it gives us a center value: the median. It also tells where the middle 50% of the values lie along the full range of values (in our example, approximately 50% of the values lie between 9.5 and 14.5). This range is referred to as the **interquartile** range. Note that this is only an approximation given that some datasets my not lend themselves well to defining 50% of its central values. In our example, only four data points fall within the interquartile range.
...
It is also desirable to find a value associated with a quantile that might not necessarily match an exact value in our batch. For example, there is no value in `a` associated with a quantile of $0.5$; this is because we have an even number of values in our dataset. The solution is to interpolate (or extrapolate) a values based on a desired quantile. The `quantile()` function does just that:

```{r}
quantile( a, 0.5)
```

If we want to get quantile values for a range of fractions, simply pass the values in a combine function:
...
A batch's quantile is best viewed as a plot where we plot the data as a function of the $f$-values:
...
```{r fig.height=3, fig.width=3}
ggplot() + aes( sample=a) + stat_qq(distribution = qunif) +xlab("f-value")
```

Note the slight change in parameter values to `ggplot()`: we are not passing a dataframe to `ggplot` but simply passing an object, `sample = a` to the `aes()` function.
