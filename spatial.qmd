...
Many mapping operations require that the data table be joined to an existing spatial dataset. If your data are aggregated at some administrative boundary level, you can find boundary files for most countries (and at different aggregate levels) in an `sf` format at [https://gadm.org/](https://gadm.org/).
...
While `ggplot2` can handle basic mapping operations, it does not offer the ease and flexibility of the `tmap` package. Much like `ggplot2`, `tmap` adopts a layering strategy. For example, to construct the above plot, we first specify the layer to be plotted via the call to `tm_shape`, then add the geometry that symbolizes the spatial feature, `tm_fill()`.
...
To change the projection, pass the EPSG projection number string to the `projection` parameter. 

To move the legend box outside of the map, set `tm_legend(outside = TRUE)`.
...
You can modify the classification breaks and colors. For example, to break the values following quantiles, and assign the same color scheme used with the ggplot output in an earlier code chunk, you would write:
