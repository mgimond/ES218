...
Date values can be represented in tables as numbers or characters. But to be properly interpreted by R as dates, date values should be converted to an R **date** object. R provides many facilities to convert and manipulate dates and times, but a package called `lubridate` makes working with dates/times much easier. The following subsections will make use of the `lubridate` package.
...
The telltale sign that the element is recognized as a date is to run the element through `str()`:

```{r}
str(x.date)
```

The keywords you are looking for are `POSIXct` or `POSIXt` which define the object's **class**. Note that R stores the date object as a number (double precision)--in fact, if you run the date object though the functions `typeof()` or `mode()` you'll see that the dates are indeed stored as numbers. An object's *class* is a wrapper around that object that helps R figure out what to do with it. You can find an object's class by wrapping it with the `class` function.

```{r}
class(x.date)
```

So even though the dates are stored as numbers R will know to treat them as dates.
...
`lubridate` does not have a function like `ym` to convert only year-month strings, this requires that we add an artificial day of the month to the string. We'll choose to add the 15th day of the month as in 
...
Note the POSIX class designation in both outputs.
...
You'll note that this last call not only returns the day of the week for each element (`r as.character(wday(x.date, label = TRUE))`) but it also returns `levels` information along with its hierarchy.
