...
You can plot multiple datasets on the same ggplot2 canvas by assigning each geom its own dataset and aesthetics. In the following example, we create two separate data frames: one for barley and another for oats. Each dataset is referenced in a separate `geom_line()` call.
