...
---
# Compare batches: the empirical QQ plot
...
To overcome the problem of batch size mismatch, we limit the number of points in the QQ plot to the number of values associated with the *smallest* sized batch (8 in this working example). This requires that we find the `a` quantiles for the set of $f$-values associated with batch `b` via the [*interpolation techniques* covered in the previous chapter](http://localhost:4724/univariate_plots.html#interpolating-a-quantile). This results in the $f$-quantile plot shown on the right hand side of the figure. The interpolated batch is labelled `a*` that now has the same number of observations as `b` thus the same set of $f$-values.
...
The multiplier can be gleaned from the graph by estimating the slope defining the point pattern. For example, a multiplier of 2 seems to do a good job in aligning the QQ points along the $x=y$ line. We can thus define the relationship as being `batch2 = batch1 * 2`. A QQ plot of `batch2` vs `batch1 * 2` is shown next.
...
The $x=y$ line in the QQ plot becomes the $y=0$ line. Deviations from this line, measured along the $y$-axis represent the additive offset observed between batches. To account for the multiplicative offset, the slope defined by the point pattern in the mean-difference plot must be transformed before applying it to the QQ plot, as follows:
...
\begin{cases}
    m' = 0, & m = 1 \\
    m' = 2, & m = \frac{4}{0} \quad \text{(undefined, vertical line in QQ plot)} \\
    m' \to \infty, & m \approx -1
\end{cases}
$$

where $m'$ is the slope defined by the point pattern in the mean-difference plot and $m$ is the corresponding slope in the QQ plot.
...
In this chapter, you learned how to compare two or more batches of data using empirical quantile–quantile (QQ) plots. Unlike side-by-side boxplots or density overlays, QQ plots allow for direct visual comparison of quantiles, making it easier to detect differences in location, spread, and shape. This approach emphasizes the importance of comparing distributions holistically rather than relying solely on summary statistics.