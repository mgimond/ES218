...
```{r echo=FALSE, fig.width=7,fig.height=3.5, results = "hide"}
OP <- par(mfrow=c(1,2))
 eda_qq(batch1, batch2 , fx = "x * 1.5 + 4", show.par = FALSE, xlab = "batch1 * 1.5 + 4")
...
## So how to the batches of singer heights compare?

We've learned that the `Tenor 1` and `Bass 2` batches differ only by location (i.e. central value) and not by spread (i.e. shape of their distribution). How do the other singer groups compare? This requires making 28 comparisons, something that is best done by generating a plot matrix.


```{r fig.height = 7, fig.width = 7}
min_size <- min(tapply(df$height, df$voice.part, length)) # get smallest batch size
singer_split <- split(df$height, df$voice.part) # Split singers into groups
rng <- range(df$height) # Get range of height values
...
The point pattern looks different here than it does in the matrix, this is because the batch sizes between `Tenor1` and `Tenor2` are the same and the `eda_qq` function will not interpolate the values. The matrix operation forces an interpolation using the `quantile` function even though none is needed. Hence, the quantile function will add a fractional component to some of the values.
