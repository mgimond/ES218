...
```{r echo=FALSE, fig.width=2,fig.height=2}
ggplot(s1, aes(batch1 * 1.5 , batch2)) + geom_point() + geom_abline( intercept=0, slope=1) +
                       coord_fixed(xlim=c(10,30), ylim = c(10,30))
```

Sometimes, you might encounter a relationship that is both additive *and* multiplicative in which case you should first resolve the multiplicative part of the pattern until the points are close to parallel to the 45&deg; line, then resolve the additive portion. For example, the following plot
...
```{r echo=FALSE, fig.width=2,fig.height=2}
ggplot(s1, aes(batch1 * 1.5 + 3.5,batch2 )) + geom_point() + geom_abline( intercept=0, slope=1) +
                                 coord_fixed(xlim=c(10,32), ylim = c(10,32))
``` 
