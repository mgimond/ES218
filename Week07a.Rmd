...
So far, we've learned how to fit a model to the data then examine the residuals for homogeneity and normality. But we haven't assessed if the differences in location (means) between batches is *significant*. This begs the question, what constitutes significant?
...
The spread between each batch's fitted mean is small compared to that of the combined residuals. So the contribution of the mean in distinguishing batch `a` from batch `b` is minimal.
