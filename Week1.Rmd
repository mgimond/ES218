...
At first glance, this may not look any different from our 3rd order polynomial. But the resulting residual suggests that the the LOESS smooth did a good job in removing any overall trend in our batch of CO2 values.
...
Let's now focus on the residual. Note the y axis values: they are three orders of magnitude less that the overall CO2 values. This indicates the oscillating nature about the overall trend is relatively small (the CO2 values have a range of [`r range(d2$CO2)`] whereas the residuals have a range of [`r round(range(d2$Residuals),2)`].
...
Note that now, all the boxplots are ligned up along their median values. We are now exploring the data after having accounted for both overall trend and seasonality. What can be gleaned form this dataset? We may want to explore the skewed nature of the residuals in February, or the narrower range of CO2 values for the fall months, for example. Note that we could have started off the analysis by removing the seasonal component from the data, then analyzed the long term trend as demontsrated by Cleveland on page 165 of the course's text[^2].
...
William Cleveland has written extensively about data visualization and has focused on principles founded in the field of cognitive neuroscience to improve data graphic designs. His book, *Visualizing Data*[^2], is a leading authority on statistical graphics and, despite its age, is as much relevant today as it was two decades ago. It focuses on graphical techniques (some newer than others) designed to explore the data and not necessarily for public dissemination which benifits from another form of data visualization called *information visualization* (or InfoVis for short). The latter will not be covered in this course. For a good discussion on the differences between statistical graphics  and InfoVis  see the 2011 Statistical Computing and Graphics Newsletter [Statistical Graphics and InfoVis: separated Twins at Birth?](http://stat-computing.org/newsletter/issues/scgn-22-1.pdf)[^3]
...
Cleveland has also contributed a very important tool to EDA: the *LOESS* curve (which was demonstrated in the CO2 data example). The LOESS curve will be used extensively in this course as will its cousin, the seasonal LOESS.

The LOESS is one of many fitting options used in smoothing (or detrending) the data. Others include paramatric models such as the family of linear polynomials (we explored two of them in the CO2 example) and Tukey's suite of smoothers notably the *running median* and the *3RS3R*. We'll be exploring these techniques throughout the course.

# We need a good data analysis environment
Effective EDA requires a flexible data analysis environment that does not constrain one to a limited set of data manipulation procedures or visualization tools. After all, would any good writer limit herself to a set of a hundred pre-built sentences? Of course not--we would be reading the same novels over and over again! So why would we limit ourselves to a limited set of pre-packaged data analysis procedures? EDA requires an arsenal of data analysis building blocks much like a good writer needs an arsenal of words. Such an environment must provide us with a flexible data manipulation environment, a flexible data visualization environment and access to a wide range of statistical procedures. A scripting environment, like R, provides us with such an environment. 
...
## Data manipulation
The importance of data visualization was already discussed in an earlier section. But before one can beginning plotting data, one must have a data table in a form ready to be plotted. In cases where the data table is composed of just two variables (columns), little data manipulation may be needed, but in cases where data tables consist of tens or scores of variables, data manipulation, subsetting and/or reshaping may be required. Tackling such a task can be challenging in a point and click spreadsheet environment and can introduce clerical error. A scripting environment like R provides an array of data maniuplation tools; furthermore, the scripting environment enables one to *read* through each step of a manipulation procedure in a clear and unambaiguous way. Imagine the difficulty in documenting in a clear and concise way the point-and-click procedures followed in a spreadsheet environment.

For example, a data table of grain production for the North America may consist of six variables. The following table shows just the first 7 lines of 1501 rows.
```{r echo=FALSE}
...
Data table manipulation is a common workflow in data analysis and, as discussed in the last section, can be prone to clerical errors if performed in a point-and-click environment. Furthermore, reproducing that workflow in a spreadsheet environment would be difficult unless each click and each copy-and-pastes were meticulously documented. And even if the documentation was adequate, there is no way of knowing if the analyst followed those exact procedures (unless his mouse and keyboard moves were recorded). However, with a scripting environment, the steps taken are clearly and unambiguously laid out. This leads to another important element of data analysis: **reproducibiliy**.
