...
The quantile need not be limited to existing values in a batch. The relationship between the  quantile $q(f)$ and the fraction $f$ is a continuous function--we refer to this relationship as the quantile function. As such, one can extract a quantile value for any fraction between 0 and 1.  Continuing with the working example, we can find the matching quantile for an $f$ = 0.5 by interpolating between the two nearest points in `a`. This results with a quantile value of 12.5.
...
Computing the quantile first requires the $f$ value for which the quantile is to be compted. The $f$ value is computed by first sorting the bacth of values from smallest to largest.
