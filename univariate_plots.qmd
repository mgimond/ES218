...
(Note that because `ggplot` requires a dataframe as input, we have to create a dataframe from vector `a`.)
...
If you wish to implement different $f$-value calculations in `ggplot` ($f$-values will be introduced in the next section), you will need to create a custom function. For example, if you wish to adopt Cleveland's quantile method ( `type = 5` in the `quantile` function) type the following:
