---
title: "Extarcting and replacing with base functions"
editor_options: 
...
To subset a dataframe by index, define both dimension's index. For example, to extract the first 20 rows and the first and third columns of the built-in `mtcars` dataset type.
...
![](img/logical_mask.png){width="262"} 
...
We can adopt the same masking properties of logical variables to **replace** values in a vector.  For example, to replace all instances of `a` in vector `x` with `z`, we first *expose* the elements equal to `a`, then assign a new value to those elements.

```{r}
x[ x == "a" ] <- "z"
x
```
![](img/logical_mask_replace.png){width="262"} 

You can think of the logical *mask* as a template applied to a street surface and a spray can spraying "z" over the template. Only the exposed portion of the street surface will be sprayed with the "z" values.
...
Note that we had to specify the column into which we are replacing the values. Had we left the second index empty, we would have replaced values across *all* columns.
