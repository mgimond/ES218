...
(Note the use of the index `dat1w[, 2:5]` which instructs R to plot columns 2 through 5). The boxplot can be ordered by first computing the median for each column then passing the sorted list of column indices to the boxplot function:

```{r fig.height=3, fig.width=5, echo=2:3}
OP <- par(mar=c(2,2,1,1))
crop.med <- apply(dat1w[, 2:5], 2, median)
boxplot(dat1w[, order(crop.med) + 1])
par(OP)
```

The first line of code computes the median values for all but the first column (Year) in `dat1w`. This results in a vector of median crop yield values, `crop.med`, where index 1 is `r names(crop.med[1])` and not `Year` as is the case in the `dat1w` data frame (this is why the object `crop.med` is incremented by `+1` in the call to `boxplot`--to ensure that the `r names(crop.med[1])` column is not treated as the "first" column in `dat1w`).
