...
eda_qq(`unit normal`, x, show.par = FALSE, q = FALSE) 
```
...
There are many ways one can fit a line to the data, Here, we opt to fit a line to the first and third quartiles (IQR) of the q-q plot. We are also adopting the [f-value type of `5`](univariate_plots.html#computing-the-f-quantile) by setting `qtype = 5` in the call to `qqline`. But note that in most cases, the choice of quantile type will have little importance in discerning the straightness of the point pattern.
