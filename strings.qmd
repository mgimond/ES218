...
To check if object `a` contains the pattern `"c d"` (note the space between the letters) anywhere in its string, use the `str_detect` function as follows:
...
The following example checks for the pattern `"cd"` (without the space):

```{r}
str_detect(a, "cd")
```

To match against multiple patterns, separate them using the "|"  (OR) operator. For example, to search for `"cd"`, `"c-d"` or `"c d"`, type:

```{r}
str_detect(a, "cd|c-d|c d")
```
...
The `%in%` operator is described in greater detail in the [next](boolean.html#the-match-operator-in) chapter.
...
## Collapsing a vector into a a single string
&nbsp;

When working with character vectors in R, you may need to collapse them into a single string. This can be done using base R’s `paste()` or stringr’s `str_c()`.

### Using base R `paste()`

The `paste()` function includes a collapse argument that concatenates elements into a single string, separating them with a specified delimiter. For example, to collapse the first five elements of the built-in `state.abb`, type:

```{r}
paste(state.abb[1:5], collapse = ", ")
```

The collapse argument specifies the separator to use between each element of the input vector. To replace it with the `|` (OR) operator, type;

```{r}
paste(state.abb[1:5], collapse = "|")
```

### Using stringr's str_c()

The `str_c()` function covered earlier in this chapter can perform the same operation. Simply pass the vector to be collapsed instead of individual string elements.

```{r}
str_c(state.abb[1:5], collapse = ", ")
```
