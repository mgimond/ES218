...
But be careful, if the aesthetic grouping parameters `colour` or `linetype` are assigned to a variable in `aes()`, identical parameters defined in `geom_line()` will overwrite any of the aesthetic arguments. For example, if the parameters `colour=` are present in both the `aes()` and `geom_line()` functions, we get the following output:
...
ggplot(dat2c,  aes(x=F, y=M)) + 
      geom_point(alpha=0.2, pch=20, cex=0.8) + 
      ylim( c(0,150000)) +
      geom_smooth(method = "lm", col="red") + 
      facet_grid( . ~ incrng) + 
      coord_fixed() + geom_abline(intercept = 0, slope = 1, col="grey50") +
