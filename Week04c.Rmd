...
Now let's look at the same data but this time conditioned on educational attainment. In the call to `ggplot`, we will limit the data to all cases with an educational attainment different from `All` (i.e. we will not want to create a plot for all educational attainment).

```{r fig.height=3, fig.width=10, warning=FALSE}
# Plot M vs F by educational attainment except for Level == All
ggplot(dat2c[dat2c$Level != "All", ]) + aes(x=F, y=M) + geom_point(alpha=0.2, pch=20, cex=0.8) + 
