...
So, what can we glean from this dataset other than the raw values associated with each occupation type? For starters, we may be interested in coming up with a summary of income values such as the median; this gives us $`r sprintf("%i",median(df$Income))`. This is a single value that can be used to characterize the overall income value. However, the income values in our batch are not all equal to $`r sprintf("%i",median(df$Income))`--they vary above or below this median value. Next, we subtract the median income value from each occupation income value to see by how much each occupation's income differs from the overall income. We can do this in a table, but will choose a dotplot chart instead to facilitate comparison.
...
A two-way table is an extension of univariate analyses where instead of grouping continuous variable  into a single factor category we explore the variable across **two** factors.
...
Note that in addition to computing the row median values for the response variable, were are also computing the median value for the *column effects* in the upper margin (which happens to be zero since we have no column effect values yet).
...
Next, we compute the row medians from the residuals, then add the *column effect* median to the top margin (and the common value) and subract the row medians from the residuals. as in step 3.
