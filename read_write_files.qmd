...
## Inspecting and summarizing tables

Once a dataset has been read into R, it’s often helpful to take a quick look at its structure and contents before proceeding with further analysis. Base R offers a few simple functions that provide an overview of a dataframe’s variables, data types, and basic summary statistics.

The `str()` function gives a compact summary of an object, including:

- The number of observations and variables;
- Each variable’s data type (numeric, character, factor, date, etc.);
- A preview of the first few values in each variable.

```{r echo = FALSE}
dat <- airquality
dat$Date <- as.Date(paste(1973, dat$Month, dat$Day, sep = "-"))
dat$Group <- factor(ifelse(dat$Month < 7, "Early", "Late"))
dat$Site  <- c("A","B")[ (seq_len(nrow(dat)) %% 2) + 1 ]
dat <- dat[, c("Date", "Wind", "Ozone", "Group", "Site")]
```
...
The `summary()` function produces basic descriptive statistics for each column:

- For **numeric** variables: minimum, quartiles, median, mean, maximum
- For **factors**: counts of each level
- For **characters**: length and type information
- For **dates**: earliest and latest values

```{r}
str(dat)
```

Two additional helper functions can be useful when checking a newly loaded dataset:
...
These functions display the first and last parts of a data object.
